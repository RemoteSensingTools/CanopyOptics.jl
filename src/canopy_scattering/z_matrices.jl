...
function _component_G_weights(components::Tuple, μ::AbstractVector)
    # Each G_c is already integrated over that component's inclination PDF;
    # only area-index weighting remains here.
...
function compute_Z_matrices_aniso_analytic(mod::SpecularCanopyScattering,
                                           μ::AbstractVector{FT},
                                           LD::AbstractLeafDistribution,
                                           m_max::Int;
                                           quadrature::CanopyQuadrature = CanopyQuadrature(),
